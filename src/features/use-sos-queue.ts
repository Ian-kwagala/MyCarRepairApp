...
// Sends an SOS that was saved while offline, as soon as the phone reconnects.

/** Replays a queued offline SOS as soon as signal returns (X1 auto-retry). */
export function useSosQueueProcessor() {
  const online = useOnline();
  const user = useUser();
  // Prevents sending the same queued SOS twice if the effect re-runs mid-send.
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // Only send an SOS queued by the user who is signed in now.
        ...
        // Still offline: keep it queued for the next reconnect. Any other error is final, so drop it and say why.
        if (!isNetworkError(e)) {
          await sosQueue.clear();
